...
from enigma import eWidget, eLabel, eTimer, ePoint, eSize, gFont, fontRenderClass, \
...
class COCRunningText(Renderer):
    def __init__(self):
        ...
        self.X = self.Y = self.W = self.H = self.W2 = self.H2 = self.mStartDelay = 0
            ...
            for (attrib, value) in self.skinAttributes:
                if attrib == "font":
                    ...
                    self.txtflags |= {"left": RT_HALIGN_LEFT, "center": RT_HALIGN_CENTER,
                                      "right": RT_HALIGN_RIGHT, "block": RT_HALIGN_BLOCK}[value]
                elif attrib == "noWrap":
    ...
    def moveLabel(self, X, Y):
        self.scroll_label.move(
            ePoint(X - self.soffset[0], Y - self.soffset[1]))
    ...
    def calcMoving(self):
        ...
        if not self.txtflags & RT_WRAP:
            self.txtext = self.txtext.replace("\xe0\x8a", " ").replace(
                chr(0x8A), " ").replace("\n", " ").replace("\r", " ")
        ...
        if self.A <= self.P <= self.B:
            ...
            if (self.mStop is not None) and (self.mStop + abs(self.mStep) > self.P >= self.mStop):
                if (self.type == RUNNING) and (self.mOneShot > 0):
                    if (self.mRepeat > 0) and (self.mCount <= 1):
                        return
