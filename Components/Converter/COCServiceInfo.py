...
class COCServiceInfo(Converter):
    ...
    def __init__(self, atype):
        ...
        self.type, self.interesting_events = {
            ...
            # compatibility to older enigma2-versions
            "VideoCodec": (self.VIDEO_TYPE, (iPlayableService.evVideoTypeReady,)),
    ...
    def getBoolean(self):
        ...
        if self.type == self.IS_STREAM:
            sref = eServiceReference(info.getInfoString(
    ...
    def getText(self):
        ...
        if self.type == self.VIDEO_PARAMS:
            ...
            print("yres", yres, "frame_rate",
                  frame_rate, "progressive", progressive)
            if not progressive:
                frame_rate *= 2
        ...
        if self.type in (self.FREQUENCY, self.SATPOSITION, self.TPDATA):
            ...
            tp_data = info.getInfoObject(iServiceInformation.sTransponderData)
            if tp_data:
                isTerrestrial = tp_data["tuner_type"] in (
                    FE.feTerrestrial, FE.feTerrestrial2)
                try:
